...
data class GestureConfig(
    ...
    val directionToleranceDegrees: Float = 65f,
    ...
    // Legacy — kept for backward compat, prefer per-edge values
    @Deprecated("Use leftDamping / rightDamping / bottomDamping")
    val dampingFactor: Float = 2.0f,
    @Deprecated("Use per-edge swipeThresholdDp")
    ...
    val pauseOnFullScreen: Boolean = false,
    val pauseOnPermissionScreen: Boolean = true,
) {
    /** Get damping factor for a specific edge. */
    fun dampingFor(edge: com.omer.akisgesture.overlay.Edge): Float = when (edge) {
        com.omer.akisgesture.overlay.Edge.LEFT -> leftDamping
        com.omer.akisgesture.overlay.Edge.RIGHT -> rightDamping
        com.omer.akisgesture.overlay.Edge.BOTTOM -> bottomDamping
    }

    /** Get swipe threshold in px for a specific edge (caller must supply dp→px value). */
    fun swipeThresholdDpFor(edge: com.omer.akisgesture.overlay.Edge): Float = when (edge) {
        com.omer.akisgesture.overlay.Edge.LEFT -> leftSwipeThresholdDp
        com.omer.akisgesture.overlay.Edge.RIGHT -> rightSwipeThresholdDp
        com.omer.akisgesture.overlay.Edge.BOTTOM -> bottomSwipeThresholdDp
    }

    /** Get vertical range for side edge. Returns null for bottom edge. */
    fun verticalRangeFor(edge: com.omer.akisgesture.overlay.Edge): Pair<Float, Float>? = when (edge) {
        com.omer.akisgesture.overlay.Edge.LEFT -> leftVerticalStart to leftVerticalEnd
        com.omer.akisgesture.overlay.Edge.RIGHT -> rightVerticalStart to rightVerticalEnd
        com.omer.akisgesture.overlay.Edge.BOTTOM -> null
    }

    /** Get trigger width/height in dp for a specific edge. */
    fun triggerSizeDpFor(edge: com.omer.akisgesture.overlay.Edge): Float = when (edge) {
        com.omer.akisgesture.overlay.Edge.LEFT -> leftTriggerWidthDp
        com.omer.akisgesture.overlay.Edge.RIGHT -> rightTriggerWidthDp
        com.omer.akisgesture.overlay.Edge.BOTTOM -> bottomTriggerHeightDp
    }
