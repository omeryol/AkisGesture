...
class WaterSurfaceModule : NaturalAnimationModule {
    ...
    override fun draw(f: AnimationFrame) {
        ...
        val span = (4f + growth * (270f + f.surfaceTension * 80f)) * f.size
        val depth = (f.stretch * (1.18f + f.surfaceTension * .32f)).coerceAtMost(400f * f.size)
        val timeSec = f.time
        ...
        auraPaint.shader = auraGradient(f, auraDepth, withAlpha(f.color, (90 * f.opacity).toInt()))
        f.canvas.drawPath(auraPath, auraPaint)

        // ── 2. LAYER: Organic Multi-Harmonic Water Body ──
        mainPath.reset()
        crestPath.reset()
        val steps = 50
        for (i in 0..steps) {
            ...
            val ripple1 = sin(u * PI * (3.2 + f.surfaceTension * 1.2) + timeSec * (2.2 + f.damping * 1.5)) * (3.0 + growth * (12.0 + f.surfaceTension * 6.0))
            val ripple2 = sin(u * PI * 8.0 - timeSec * (1.8 + f.viscosity * 2.0)) * growth * (4.0 + f.viscosity * 5.0)
        ...
        mainPaint.shader = gradient(
            f, depth,
            withAlpha(brightColor, (240 * f.opacity).toInt()),
            withAlpha(deepColor, (190 * f.opacity).toInt())
        )
        f.canvas.drawPath(mainPath, mainPaint)

        // ── 3. LAYER: Foreground Specular Highlight Crest ──
        val crestColor = lighten(f.color, 0.85f)
        crestPaint.strokeWidth = (2.5f + growth * 2f) * f.size
        crestPaint.color = withAlpha(crestColor, (220 * f.opacity).toInt())
